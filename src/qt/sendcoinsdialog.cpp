    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Ingot address (e.g. NgJKYsHctQfF8D8r6otTGxdnB6r311NvJf)"));
    //TX Comment
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (!model->getOptionsModel() || !model->getOptionsModel()->getCoinControlFeatures())
        sendstatus = model->sendCoins(txcomment, recipients); // TX Comment
    else
        sendstatus = model->sendCoins(txcomment, recipients, CoinControlDialog::coinControl); // TX Comment
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Ingot address"));
        }
        else
        {
