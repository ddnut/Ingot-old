...
{
    // Convert Ingot:// to Ingot:
    //
    //    Cannot handle this later, because Ingot:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Ingot://"))
    {
        uri.replace(0, 12, "Ingot:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ingot.lnk";
}
...
{
    return GetAutostartDir() / "Ingot.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Ingot-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Ingot-qt [" + tr("command-line options") + "]                     " + "\n";
